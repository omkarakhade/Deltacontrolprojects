...
export default function CategoryPlate({ category, index }) {
  return (
    <Link
      ...
      className="spec-plate relative rounded-sm p-5 flex flex-col justify-between h-full text-white group overflow-hidden hover:ring-1 hover:ring-accent-500 transition-all"
    >
      ...
      <div>
        <span className="font-mono text-[10px] text-accent-400 tracking-widest">
          {String(index + 1).padStart(2, "0")} / {category.tag}
        </span>
        <h3 className="font-display text-xl mt-2 leading-tight group-hover:text-accent-400 transition-colors">
          {category.name}
        </h3>
      </div>
